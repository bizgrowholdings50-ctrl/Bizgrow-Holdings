...
export async function GET(request) {
  ...
  if (code) {
    ...
    if (user) {
      ...
      console.log(
        "Query Name: adminClient.profiles.select(referral_code, email, full_name, avatar_url).eq(id, auth.uid())",
      );
      ...
      console.log(
        "- Returned error:",
        profileError ? formatSupabaseError(profileError) : null,
      );
      ...
      if (!profileData) {
        console.log(
          "- Profile does not exist. Creating new profile. auth.uid() =",
          authUid,
        );
        const newProfile = {
          id: authUid,
        ...
        console.log(
          "- Returned error:",
          insertError ? formatSupabaseError(insertError) : null,
        );
        ...
        console.log(
          "- Profile exists but lacks referral code. Updating. auth.uid() =",
          authUid,
        );
        ...
        console.log(
          "Query Name: adminClient.profiles.update(updatePayload).eq(id, auth.uid())",
        );
        ...
        console.log(
          "- Returned error:",
          updateError ? formatSupabaseError(updateError) : null,
        );
      ...
      if (referrerCode) {
        ...
        if (referrerProfile?.id) {
          ...
          if (referrerProfile.id === authUid) {
            ...
            if (existingReferral) {
              console.log(
                "STEP 9 Decision: Duplicate referral skipped for referred_user_id:",
                authUid,
              );
            } else {
              console.log("STEP 10: Inserting new referral record");
