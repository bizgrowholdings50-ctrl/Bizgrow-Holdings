...
const videoData = [
  { id: 1, name: "Alaiba", role: "Speaker", company: "BizGrow Partner", videoUrl: "/video/Alaiba.mp4" },
  { id: 2, name: "Workshop Team", role: "Strategic Team", company: "BizGrow Holdings", videoUrl: "/video/Workshop-video-2.mp4" },
  { id: 3, name: "James Dalton", role: "CEO", company: "SafeGuard Logistics", videoUrl: "/video/ts-1.mp4" },
];
  ...
  return (
    <section className="py-12 md:py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        ...
        {/* --- HEADING SECTION: Only BizGrow Holdings --- */}
        <div className="text-center mb-16 max-w-3xl mx-auto">
          ...
          >
            Watch how BizGrow Holdings is transforming industries through innovation and dedicated partnership.
          </motion.p>
        </div>
        ...
        <div className="relative w-full aspect-video min-h-[450px] md:min-h-[650px] rounded-[2.5rem] md:rounded-[4rem] overflow-hidden bg-[#0c054e] shadow-2xl isolate">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <video
                ...
                className="absolute inset-0 w-full h-full object-cover opacity-40 blur-[50px] scale-110"
              >
                <source src={videoData[index].videoUrl} type="video/mp4" />
              </video>

              {/* 2. CENTER PORTRAIT VIDEO */}
              ...
              <div className="absolute inset-0 p-6 md:p-16 flex flex-col justify-between z-30 pointer-events-none">
                <div className="bg-white/10 backdrop-blur-md self-start px-4 py-1 rounded-full border border-white/20 pointer-events-auto">
                  <span className="text-white text-[10px] font-bold uppercase tracking-widest">Success Story</span>
                </div>
                ...
                <div className="flex flex-col md:flex-row items-end justify-between gap-6 pointer-events-auto">
                  <div className="text-white drop-shadow-lg">
                    ...
                    <p className="text-[#997819] font-bold text-xs md:text-xl uppercase tracking-wider">
                      {videoData[index].role} <span className="mx-2 opacity-50">|</span> {videoData[index].company}
                    </p>
                  </div>
          ...
          <div className="absolute top-1/2 -translate-y-1/2 w-full flex justify-between px-4 md:px-8 z-40 pointer-events-none">
            <button onClick={prev} className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-white/10 backdrop-blur-md text-white flex items-center justify-center hover:bg-[#997819] transition-all pointer-events-auto border border-white/10"><ChevronLeft /></button>
            <button onClick={next} className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-white/10 backdrop-blur-md text-white flex items-center justify-center hover:bg-[#997819] transition-all pointer-events-auto border border-white/10"><ChevronRight /></button>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {playingVideo && (
          ...
          >
            <button onClick={() => setPlayingVideo(null)} className="absolute top-6 right-6 text-white hover:text-[#997819] transition-colors"><X size={40} /></button>
            <div className="w-full max-w-5xl aspect-video">
              <video autoPlay controls className="w-full h-full object-contain"><source src={playingVideo} type="video/mp4" /></video>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
